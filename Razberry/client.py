...
if __name__ == '__main__':
    ...
    while True:
        ...
        img = raz_module.iniaialize_picam()                 #return gray
        raz_ui.initalize_UI()

        if (raz_module.detect(img, cascade) != 1):
            rects = raz_module.detect(img, cascade)
            vis = img.copy()
            raz_module.draw_rects(vis, rects, (0, 255, 0))
            ...
            while True:
                ...
                else:                                        #다른 response가 올 경우 내부 while continue
                    time.sleep(2)
                    continue
        else:
            time.sleep(2)
